...
export default function Home() {
  ...
  const { isListening, isSpeaking, sttSupported, speak, toggleListening } = useVoice({
  ...
  return (
    <div className="flex h-[100dvh] bg-[#1a2a6c]">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        {/* Mobile header */}
        <header className="lg:hidden flex items-center justify-between px-3 py-3 bg-[#1a2a6c] border-b border-white/10">
          <button type="button" onClick={() => setWelcomeOpen(true)} className="flex items-center gap-2.5 hover:opacity-80 transition-opacity">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <div className="w-11 h-11 rounded-full overflow-hidden border-2 border-[#00b894] flex-shrink-0">
              <img src="/avatar-tutor.png" alt="Tutor" className="w-full h-full object-cover" />
            </div>
            <div className="text-left">
              <p className="text-sm font-bold text-white">ROHU</p>
              <p className="text-[11px] text-[#00b894] font-medium">Learn English</p>
            </div>
          </button>
          <div className="flex items-center gap-2">
            {progress.streak > 0 && <span className="text-xs text-white">🔥{progress.streak}</span>}
            <span className="text-[10px] bg-white/10 rounded-full px-2 py-1 text-white font-bold">
              {progress.level.emoji} {progress.todayXp}XP
            </span>
            <button type="button" onClick={() => setQuizOpen(true)}
              className="w-9 h-9 flex items-center justify-center rounded-lg bg-[#00b894] text-white text-base active:scale-90">
              🎯
            </button>
            <button type="button" onClick={() => setListeningQuizOpen(true)}
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-zinc-900 overflow-hidden lg:rounded-tl-2xl">
          ...
          <InputBar onSend={sendMessage} onMicClick={toggleListening} disabled={isStreaming} isListening={isListening} showMic={sttSupported} />
        </div>
      </main>
